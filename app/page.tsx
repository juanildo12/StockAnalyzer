...
export default function Home() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', width: '100%', maxWidth: '100%', background: C.bg, color: C.textSecondary, fontFamily: 'system-ui, -apple-system, sans-serif' }}>
          ...
          <>
            ...
            {data && (
              ...
              <div style={{ marginTop: S.md }}>
                <ValuationGauge symbol={data.quote.symbol} />
              </div>
            </>
            )}
          </>
      ...
      {view === 'classification' && (
        <div style={{ animation: 'slideUp 0.25s cubic-bezier(0.16, 1, 0.3, 1) forwards' }}>
          <StockClassification onSelect={(sym) => { window.open(`/?symbol=${sym}`, '_blank', 'noopener,noreferrer'); }} />
        </div>
      )}
...
function RiskReport({ data, symbol }: { data: ApiResponse; symbol: string }) {
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
      ...
      <OnboardingModal onSelectSymbol={(sym) => { setSymbol(sym); searchStock(sym); }} />
    </div>
  );
}
