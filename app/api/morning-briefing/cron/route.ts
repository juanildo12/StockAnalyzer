...
export async function GET(request: Request) {
  try {
    ...
    console.log('[Briefing Cron] Starting full scan...');
    ...
    console.log(`[Briefing Cron] Universe: ${universe.length} stocks`);
    ...
    console.log(`[Briefing Cron] Got quotes for ${rows.length} stocks`);
    ...
    console.log(`[Briefing Cron] ${enriched.length} breakout candidates found`);
    ...
    await cacheSet('briefing:live:current', responseData, 21600);
    console.log(`[Briefing Cron] Done. Cached ${finalPicks.length} picks for 6h.`);

    return NextResponse.json({ ok: true, picks: finalPicks.length, scanned: rows.length });
  } catch (error) {
