...
export function generateInformeDetail(input: InformeInput): InformeDetail {
  ...
  return {
    ...
    strategy: {
      ...
      target1: `+${((tipRanksPriceTarget / currentPrice - 1) * 100).toFixed(1)}% → $${tipRanksPriceTarget.toFixed(2)}`,
      target1Value: tipRanksPriceTarget,
      target2: `+${((myProjectionHigh / currentPrice - 1) * 100).toFixed(1)}% → $${myProjectionHigh.toFixed(2)}`,
      target2Value: myProjectionHigh,
      stopLoss: `-${((1 - (currentPrice * 0.80) / currentPrice) * 100).toFixed(1)}% → $${(currentPrice * 0.80).toFixed(2)}`,
